...
#month1.ts <- ts(new_Uk_data_1$total_cases,start = c(2020),end=c(2021),frequency = 1)

#month2.ts <- ts(new_Uk_data_1$stringency_index,start = c(2020,1), end = c(2020,6),frequency = 12)

#month3.ts <- ts(new_Uk_data_1$total_deaths,start = c(2020,2), end = c(2020,7),frequency = 5)

#month4.ts <- ts(new_Uk_data_1$reproduction_rate,start = c(2020,2), end = c(2020,7),frequency = 5)

#month5.ts <- ts(new_Uk_data_1$total_tests,start = c(2020,2), end = c(2020,7),frequency = 5)

#month6.ts <- ts(new_Uk_data_1$population,start = c(2020,2), end = c(2020,7),frequency = 5)
...
#wave vise individual effects 
#wave1.ts <- ts(new_Uk_data_1$total_cases,start = c(2020,1),end=c(2020,6),frequency = 1)
#plot(wave1.ts)

#wave1_Stringency_index.ts <- ts(,start = c(2020),end=c(2021),frequency = 1)

#now for 2020/8-2021/1
#month_new_1.ts <- ts(new_Uk_data_1$total_cases,start = c(2020,7), end = c(2020,12),frequency = 12)


#plot(month_new_1.ts)

#month_new_2.ts <- ts(new_Uk_data_1$total_cases,start = c(2021,1), end = c(2021,6),frequency = 12)

#plot(month_new_2.ts)

#checking the missing values in uk
#uk_missing_values <- lapply(Uk_data, function(x) mean(is.na(x))*100)

#View(uk_missing_values)



#Handling the uk missing values 
#na.omit(Uk_data,target.colnames="new_cases_smoothed")
#deleating the rows which have really less percentage of missing values 
#Uk_data<- Uk_data[!is.na(Uk_data$new_cases_smoothed),]
#Uk_data<- Uk_data[!is.na(Uk_data$new_cases_smoothed_per_million),]
#Uk_data<- Uk_data[!is.na(Uk_data$stringency_index),]

#now lets see the missing values %
#uk_missing_values_1 <- lapply(Uk_data, function(x) mean(is.na(x))*100)

#View(uk_missing_values_1)

#Dropping colums which are not required According to our question and creating,
#a df with the required ones 
#new_Uk_data =select(Uk_data,-c(population,population_density,
                               #median_age,aged_65_older,aged_70_older,gdp_per_capita,
                               #extreme_poverty,cardiovasc_death_rate,diabetes_prevalence,
                               #female_smokers,male_smokers,handwashing_facilities,
                               #hospital_beds_per_thousand,life_expectancy,
...
#runing mice and checking 
#library(VIM)
#mis_plot <- aggr(Uk_data, col=c('navyblue','red'), numbers=TRUE, sortVars=TRUE, 
                  #labels=names(Uk_data), cex.axis=.7,
                #gap=3, ylab=c("Histogram of missing data","Pattern"))

#uk_data_imp <-mice(new_Uk_data,m=5,maxit = 50,method = 'pmm',seed = 500) 

#
#Uk_imp_data <- na.aggregate(new_Uk_data)
#View(Uk_imp_data)

#Uk_data_2 <- Uk_data
#for(i in 1:ncol(Uk_data)) {
  #Uk_data_2[ , i][is.na(Uk_data_2[ , i])] <- mean(Uk_data_2[ , i], na.rm = TRUE)
#}

#Uk_data_2[is.na(Uk_data_2)]=0
#View(Uk_data_2)

#Since the missing values are the ones where the results are even done,
#insted of imputing , making them zero makes sence
#new_Uk_data[is.na(new_Uk_data)]=0
#View(new_Uk_data)

#Now to do correlation 
#cor(new_Uk_data)
#Uk_correaltion_data.cor=cor(new_Uk_data)

#For now lets drop the categorical colums and check what is the correlation,
#between other factors to that of stringency index

#Uk_correaltion_data=select(new_Uk_data,-c(iso_code,continent,
                                          #location,date))

#View(Uk_correaltion_data)

#head(Uk_correaltion_data,5)
#running correaltion test
#attach(Uk_correaltion_data)
#detach(Uk_correaltion_data)
#cor.test(Uk_correaltion_data$total_cases,Uk_correaltion_data$stringency_index)
#cor.test(total_cases_per_million,stringency_index)
#cor.test(new_cases,stringency_index)


#typeof(Uk_correaltion_data)

#Uk_correaltion_data.cor=cor(Uk_correaltion_data)


#time series
#month1.ts <- ts(new_Uk_data,start = c(2020,2), end = c(2020,7),frequency = 5)

#plot(month1.ts)

#now calculating the missing values of the new df###
#sum(is.na(new_covid_main))

#Since some colums have very less percentage of missing value 
#deletiing those rows

#new_covid_main <- new_covid_main[!is.na(new_covid_main$total_cases),]
#new_covid_main <- new_covid_main[!is.na(new_covid_main$new_cases),]
#new_covid_main <- new_covid_main[!is.na(new_covid_main$new_cases_smoothed),]
#new_covid_main <- new_covid_main[!is.na(new_covid_main$total_cases_per_million),]
#new_covid_main <- new_covid_main[!is.na(new_covid_main$new_cases_per_million),]
#new_covid_main <- new_covid_main[!is.na(new_covid_main$new_cases_smoothed_per_million),]

#new_covid_main <- new_covid_main[!is.na(new_covid_main$population),]


#new_covid_main <- new_covid_main[!is.na(new_covid_main$life_expectancy),]

#View(new_covid_main)
#running to see now what the missing values % is 
#missing_values_1 <- lapply(new_covid_main, function(x) mean(is.na(x))*100)
#View(missing_values_1)
...
#running imp
#uk_imp_data <- subset(Uk_data,select = -c(iso_code,
                                                 #continent,
                                                # location,
                                                 #date,
                                                 #total_cases,
...
#imputed_Data_2 <- mice(uk_imp_data,m=5,maxit = 20,
                       #method = "pmm",seed=500)
#summary(imputed_Data_2)

######-########
#imputation process
#lets see 
#md.pattern(new_covid_main)

#new_covid_main <- subset(new_covid_main,select = -c(iso_code,
                                                 #   continent,
                                                 #   location,
                                                 #   date,
                                                  #  total_cases,
...
#View(new_covid_main)
#now lets see
#imputed_set <-mice(new_covid_main,m=5,maxit = 50,method = 'pmm',
                   #seed=500)

#imputed_Data_2 <- mice(new_covid_main,m=5,maxit = 50,
                       #method = "logreg",seed=500)
#summary(imputed_Data_2)
#trying Hmisc for imputation
#using argImpute
#data_impute <- aregImpute(~total_deaths+new_deaths+new_deaths_smoothed+
 #                          total_deaths_per_million+new_deaths_per_million,
 #                         new_covid_main,n.impute = 5)


#data_impute
#data_impute$imputed$total_deaths
#data_impute$imputed$new_deaths
#data_impute$imputed$new_deaths_smoothed

#data_impute_1 <- aregImpute(~
                         # new_covid_main,n.impute = 5)

#uk_new=select(Uk_data,c(location,date,total_cases,total_deaths,
                             #   reproduction_rate,total_tests,stringency_index,
                              #  tests_per_case,))
